...
class FilterApp(QtWidgets.QMainWindow):

	def __init__(self):
		super(FilterApp, self).__init__()
		self.ui = Ui_MainWindow()
		self.ui.setupUi(self)
		self.filter = TunableFilter(order=2, center=500e6, fbw=50, ripple=3)
		self.ui.plotWidget.setBackground('w')
		self.ui.plotWidget.setTitle('Expected filter response')
		self.ui.plotWidget.setLabel('left', 'S21 [dB]')
		self.ui.plotWidget.setLabel('bottom', 'Frequency [Hz]')
		self.plot()
		#self.ui.plotWidget.setLabel('right', '')
	...
	def connectSignals(self):
		...
		self.ui.centerFreqSpinBox.valueChanged.connect(self.centerFreqChenged)
		self.ui.fbwSpinBox.valueChanged.connect(self.fbwChanged)
		self.ui.rippleSpinBox.valueChanged.connect(self.rippleChanged)
		#for i in range(0, 4):
	...
	def plot(self):
		freq, S21 = self.filter.plot()
		self.ui.plotWidget.clear()
		self.ui.plotWidget.plot(freq, S21)

	def rippleChanged(self, ripple):
		self.filter.ripple = ripple
		self.plot()

	def centerFreqChenged(self, freq):
		self.filter.setCenterFreq(freq*1e6)
		self.plot()

	def fbwChanged(self, fbw):
		self.filter.setFractBW(fbw)
		self.plot()
